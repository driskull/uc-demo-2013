define([
    ...
    function (
        ...
        return declare("", null, {
            ...
            _swipe: function (val) {
                var _self = this;
                if (_self._swipediv !== null) {
                    var offset_left = parseFloat(_self._swipediv.style.left);
                    var offset_top = parseFloat(_self._swipediv.style.top);
                    ...
                    _self._swipediv.style.clip = clipstring;
                }
            },
            _clipLayer: function () {
                var _self = this;
                //Initial swipe slider location
                _self._swipe(dom.byId("sliderdiv").style.left);

                //Make the slider visible
                dom.byId("sliderdiv").style.display = "";


                connect.connect(_self._swipeslider, 'onMove', function () {
                    this.node.style.top = "0px"; //needed to avoid offset
                    var left = parseInt(this.node.style.left, 10);
                    if (left <= 0 || left >= (_self.map.width)) {
                        return;
                    }
                    _self._clipval = this.node.style.left;
                    _self._swipe(_self._clipval);
                });

                connect.connect(_self.map, 'onPanEnd', function () {
                    _self._swipe(_self._clipval);
                });

                if (_self.map.navigationMode === "css-transforms") {
                    connect.connect(_self.map, 'onPan', function () {
                        _self._swipe(_self._clipval);
                    });
                }
            },
            _initSwipe: function (layers) {
                var _self = this;
                var title = "MooreTornado1";
                var id;
                if (layers.length) {
                ...
                _self._clipval = _self.map.width;
                _self._swipediv = null;
                _self._swipelayerid = id;
                var layer = _self.map.getLayer(id);
                if (layer === null || layer === undefined) {
                    return;
                }
                _self._swipediv = layer._div;
            },
            ...
            _createWebMap: function () {
                ...
                arcgisUtils.createMap(this.config.webmap, "mapDiv", {
                    ...
                    _self._initSwipe(response.itemInfo.itemData.operationalLayers);

                    _self._swipeslider = new move.parentConstrainedMoveable("sliderdiv", {
                        area: "content",
                        within: true
                    });

                    _self._swipeslider.node.style.height = _self.map.height + "px";
                    _self._swipeslider.node.style.top = "0px";
                    _self._swipeslider.node.style.left = "0px";
